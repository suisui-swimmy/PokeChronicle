...
When `others/champout` is present, the report also builds a Node-only review/index candidate set from `btl_set.json` and `btl_std.json`. These candidates are not active rules by themselves: labels outside the current allowlist are emitted as `sourceStatus: "review_index"` with flags such as `allowedByCurrentConfig`, `blockedByCurrentConfig`, `blockedByDenyPattern`, `requiresPlaceholderPolicy`, and `riskHints`.

This is a review aid, not an automatic fix. `btl_set.json` as a whole must not be bundled into the active generated pack. Promote only narrow label categories after checking placeholder meaning, event type, canonical display, false-positive risk, and tests, then regenerate with `npm run generate:champout-templates`. Missing `others/champout` only disables source comparison; replay metrics and parser proposals still run.
