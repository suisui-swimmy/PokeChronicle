...
- MediaDevices `getDisplayMedia()` による画面/ウィンドウ/タブのブラウザ内キャプチャ。
...
- ユーザーがブラウザで画面、ウィンドウ、またはタブをキャプチャする。
...
- `champout` 由来のテキストは、再配布可能性が明確に確認できない限り、ユーザーがブラウザ内で読み込む外部データとして扱う。
- `champout` 由来の生成済み全文テンプレートを、再配布可能性が確認できない状態で `src/` や `public/` に同梱しない。
...
- `others/champout`
  ...
  - AGENTSの `champoutからのデータimport` 方針どおり、実行時はユーザーがブラウザ内で選択・importする外部データとして扱う。
  - 再配布可能性が明確でない公式由来テキストを `src/` や `public/` に同梱しない。
...
- Project Pokémon `champout`: `https://github.com/projectpokemon/champout`
  ...
  - 静的Webアプリ実行時は、ユーザーがローカルファイルとして読み込む方式にする。
...
1. 画面、ウィンドウ、またはタブのキャプチャ映像をブラウザ上に表示し、リアルタイム解析を開始・停止できる。
...
- 画面、ウィンドウ、またはタブのキャプチャ映像をブラウザ内に表示し、バトルメッセージROIを調整できるようにする。

Scope:

- `getDisplayMedia()` によるキャプチャ開始・停止。
- キャプチャ映像のpreview表示。
...
### M7: champout/template import

Goal:

- 名前・技以外のバトル文言の分類率を上げるため、ユーザーがブラウザ内でchampout由来データを読み込めるようにする。
...
- 読み込んだJSONの再帰的文字列抽出。
- `btl_*` などバトル関連候補の優先。
- source file name、key/id、sourceCommitまたはunknownの保持。
- template候補の正規化、placeholder変換、IndexedDB保存。
- import済みtemplateの削除とexport。

Done:

- `others/champout` をruntime importせず、ユーザー選択ファイルとして読み込める。
- import件数、抽出件数、バトル関連候補件数が表示される。
- seed rules + dictionary + imported templatesで分類候補を生成できる。

Stop line:

- 再配布可否が未確認のchampout由来全文テンプレートを `src/` や `public/` にコミットしない。
...
1. ユーザーがブラウザ上で選択した `champout` 由来JSON、ZIP、または事前変換済みテンプレートJSONから、バトルテキストテンプレートをimportする。
2. 関連JSONファイルから日本語バトル関連文字列を抽出する。
3. `{0}`, `{1}` のようなplaceholderを、照合可能なplaceholderへ変換する。
4. 統計価値の高いテンプレートに対して、手書きのevent ruleを付与する。
5. マッチしなかったテンプレートは、レビューと将来の分類追加用に保持する。

生成されたテンプレートは、再配布可能性が確認できない限りリポジトリへコミットしない。ユーザーのブラウザ内IndexedDBに保存するか、ユーザーがJSONとしてダウンロードする。
...
## champoutからのデータimport

静的Webアプリでは、実行時に `../champout` のようなローカルパスを直接読むことはできない。

そのため、以下のいずれかの方式を実装する。

### 推奨: ブラウザ内import

ユーザーがファイル選択UIから、`champout` 由来のJSONファイルまたはZIPを読み込む。

要件:

- `<input type="file">` で複数JSONを読み込める。
- 可能であればZIP importに対応する。
- ZIP対応を入れる場合は、ブラウザで動くライブラリを使う。
- 読み込んだJSONを再帰的に調べ、文字列値を堅牢に抽出する。
- 確認なしに特定schemaだけを仮定しない。
- `btl_*` のようなバトル関連ファイルを優先する。
- 抽出した各文字列について、source file nameとkey/idを保持する。
- import結果はIndexedDBに保存できる。
- import結果はユーザーがJSONとしてdownloadできる。
...
```json
{
  ...
  "sourceCommit": "unknown-or-user-provided",
  "entries": [
    {
      "id": "source-key-or-path",
      "sourceFile": "rom-txt/jpn/btl_attack_syn.json",
      "text": "{0}の\n{1}！",
      "normalizedText": "{0}の\n{1}!"
    }
  ]
...
開発補助として、Node.js scriptを置いてよい。

```bash
node scripts/prepare-static-data.mjs --source ../champout --lang jpn
```

ただし、このscriptで生成した公式由来テキストを、再配布可能性が未確認のままコミットしない。

sourceがない場合やschemaが想定と異なる場合は、明確なエラーを出し、期待する入力形式をdocumentに書く。
...
- ユーザーが外部テンプレートJSONまたはchampout由来JSON/ZIPを読み込める。
- import件数、抽出件数、バトル関連候補件数を表示する。
- import済みデータをIndexedDBへ保存できる。
- import済みデータを削除できる。
...
リアルタイムキャプチャでは、`getDisplayMedia()` 自体はUI thread側で扱い、WorkerにはCanvas/OffscreenCanvas由来のフレームまたはcropを渡す。Workerはフレームごとの前処理、OCR、正規化、分類候補生成を担当し、UIはライブログとレビュー状態の管理を担当する。
...
- 画面、ウィンドウ、またはタブのキャプチャ映像をブラウザ内に表示し、リアルタイム解析を開始・停止できる。
...
3. 画面/ウィンドウ/タブのキャプチャ表示UIと開始・停止制御を実装する。
